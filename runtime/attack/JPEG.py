...
class rgb_to_ycbcr_jpeg(nn.Module):
    ...
    def forward(self, image):
        ...
        result = torch.tensordot(image, self.matrix.to(image), dims=1) + self.shift.to(image)
...
class dct_8x8(nn.Module):
    ...
    def forward(self, image):
        image = image - 128
        result = self.scale.to(image) * torch.tensordot(image, self.tensor.to(image), dims=2)
        result.view(image.shape)
        return result
